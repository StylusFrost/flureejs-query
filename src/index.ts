...
export default class Query {
  ...
  /**
   * Creates a new query from an object with its fields' values.
   *
   * @param data - A query can be initialized with object containing them by name.
   *
   * @example
   * ```js
   * const queryData = {
...
digest: SHA-256=${digest.toString('base64')}`
  ...
  /**
   * sign a query with a given private key
   * @param privateKey
   */
